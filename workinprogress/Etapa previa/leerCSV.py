...
Para eso construimos un archivo auxiliar (fileCodigos) que tiene la informacion del formulario necesaria estrcutruada en bloques. Cada bloque corresponde a una pregunta y esta separado del siguiente por una linea en blanco. Cada bloque tiene una primer linea con el texto correspondiente al titulo de la pregunta y un anexo "_M" para indicar que espera opciones multiples si corresponde. Una segunda linea que indica cual es la combinacionde indices que apuntan a la pregunta en cuestion, esta combinacion omite la iteracion sobre el ultimo indice que recorre las opciones de la pregunta en cuestion. Un conjunto arbitrariamente largo de lineas donde cada linea corresponde al titulo de cada una de las opciones incluidas en la pregunta respetando el orden establecido en el formulario. 
...
La estructura con la que trabaja el codigo es un pandas dataframe. 
